...
if ( ! class_exists( 'Ruby_Reaction', false ) ) {
	class Ruby_Reaction {
		...
		public function get_settings() {
			...
			$settings  = [];
			$reactions = gulir_get_option( 'reaction_items' );
		...
		function register_reaction() {
			...
			$this->defaults = [
				'love'      => [
					'id'    => 'love',
					'title' => gulir_html__( 'Love', 'gulir-core' ),
					'icon'  => 'icon-love',
				],
				'sad'       => [
					'id'    => 'sad',
					'title' => gulir_html__( 'Sad', 'gulir-core' ),
					'icon'  => 'icon-sad',
				],
				'happy'     => [
					'id'    => 'happy',
					'title' => gulir_html__( 'Happy', 'gulir-core' ),
					'icon'  => 'icon-happy',
				],
				'sleepy'    => [
					'id'    => 'sleepy',
					'title' => gulir_html__( 'Sleepy', 'gulir-core' ),
					'icon'  => 'icon-sleepy',
				],
				'angry'     => [
					'id'    => 'angry',
					'title' => gulir_html__( 'Angry', 'gulir-core' ),
					'icon'  => 'icon-angry',
				],
				'dead'      => [
					'id'    => 'dead',
					'title' => gulir_html__( 'Dead', 'gulir-core' ),
					'icon'  => 'icon-dead',
				],
				'wink'      => [
					'id'    => 'wink',
					'title' => gulir_html__( 'Wink', 'gulir-core' ),
					'icon'  => 'icon-wink',
				],
				'cry'       => [
					'id'    => 'cry',
					'title' => gulir_html__( 'Cry', 'gulir-core' ),
					'icon'  => 'icon-cry',
				],
				'embarrass' => [
					'id'    => 'embarrass',
					'title' => gulir_html__( 'Embarrass', 'gulir-core' ),
					'icon'  => 'icon-embarrass',
				],
				'joy'       => [
					'id'    => 'cry',
					'title' => gulir_html__( 'Joy', 'gulir-core' ),
					'icon'  => 'icon-joy',
				],
				'shy'       => [
					'id'    => 'shy',
					'title' => gulir_html__( 'Shy', 'gulir-core' ),
					'icon'  => 'icon-shy',
				],
				'surprise'  => [
					'id'    => 'surprise',
					'title' => gulir_html__( 'Surprise', 'gulir-core' ),
					'icon'  => 'icon-surprise',
				],
			];
		}
		...
		public function get_svg( $icon ) {
			...
			if ( function_exists( 'gulir_get_svg' ) ) {
				return gulir_get_svg( $icon, '', 'reaction' );
			}
		...
		function render( $attrs ) {
			...
			if ( is_array( $reactions ) && count( $reactions ) ) {
				...
				foreach ( $reactions as $reaction ) {
					...
					$output .= '<span class="reaction-title h6">' . gulir_strip_tags( $reaction['title'] ) . '</span>';
					$output .= '</span>';
					$output .= '<span class="reaction-count">';
					if ( empty( $total[ $reaction['id'] ] ) ) {
						$output .= '0';
					} else {
						$output .= gulir_pretty_number( $total[ $reaction['id'] ] );
					}
					$output .= '</span>';
					$output .= '</div>';
				}
		...
		function add_reaction() {
			...
			if ( 'add' === $type ) {
				Gulir_Personalize_Helper::get_instance()->save_reaction( $reaction, $post_id );
			} elseif ( 'delete' === $type ) {
				Gulir_Personalize_Helper::get_instance()->delete_reaction( $reaction, $post_id );
			}
