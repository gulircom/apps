...
if ( ! class_exists( 'Gulir_Personalize_Helper' ) ) {
	class Gulir_Personalize_Helper {
		...
		public function __construct() {
			...
			$this->activated     = gulir_get_option( 'bookmark_system' );
			...
			add_action( 'gulir_schedule_history_cleanup', [ $this, 'history_cleanup' ], 10 );
			add_action( 'gulir_schedule_bookmark_cleanup', [ $this, 'guest_bookmark_cleanup' ], 10 );
			add_action( 'gulir_schedule_voting_cleanup', [ $this, 'voting_cleanup' ], 20 );
			add_action( 'gulir_schedule_reaction_cleanup', [ $this, 'reaction_cleanup' ], 30 );
		...
		function get_identifier() {
			...
			if ( ! empty( $_COOKIE['RBUUID'] ) ) {
				...
				// Try to get from GET method
				return sanitize_text_field( trim( $_GET['uuid'] ) );
			}
		...
		function register_schedule() {
			...
			if ( gulir_get_option( 'reading_history' ) ) {
				if ( ! wp_next_scheduled( 'gulir_schedule_history_cleanup' ) ) {
					wp_schedule_event( time(), 'daily', 'gulir_schedule_history_cleanup' );
				}
			}

			if ( gulir_get_option( 'bookmark_system' ) ) {
				if ( ! wp_next_scheduled( 'gulir_schedule_bookmark_cleanup' ) ) {
					wp_schedule_event( time(), 'daily', 'gulir_schedule_bookmark_cleanup' );
				}
			}

			if ( gulir_get_option( 'single_post_reaction' ) ) {
				if ( ! wp_next_scheduled( 'gulir_schedule_voting_cleanup' ) ) {
					wp_schedule_event( time(), 'daily', 'gulir_schedule_voting_cleanup' );
				}
			}

			if ( ! wp_next_scheduled( 'gulir_schedule_reaction_cleanup' ) ) {
				wp_schedule_event( time(), 'daily', 'gulir_schedule_reaction_cleanup' );
			}
		}
		...
		function history_cleanup() {
			...
			$expired    = absint( gulir_get_option( 'reading_history_expired', 2 ) );
			$table_name = $this->get_table_name();
		...
		function guest_bookmark_cleanup() {
			...
			$expired = absint( gulir_get_option( 'bookmark_guest_expired', 14 ) );
		...
		public function save_vote( $reaction = 'like', $post_id = '' ) {
			...
			$table_name   = $this->get_table_name();
			$track_ip     = gulir_get_option( 'reaction_ip' );
			$action_key   = 'vote';
			$action_value = 'like';
		...
		function voting_cleanup() {
			...
			$guest_expired = absint( gulir_get_option( 'reaction_guest_expired', 14 ) );
			$user_expired  = absint( gulir_get_option( 'reaction_logged_expired', 14 ) );
...
/** load */
Gulir_Personalize_Helper::get_instance();