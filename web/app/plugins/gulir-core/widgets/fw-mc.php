...
if ( ! class_exists( 'Gulir_Fw_Mc' ) ) {
	class Gulir_Fw_Mc extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( $this->widgetID, esc_html__( 'Gulir - FW Newsletter', 'gulir-core' ), [
				'classname'   => $this->widgetID,
				'description' => esc_html__( '[Full Width Widget] Display a Mailchimp sign-up form in the full width sections.', 'gulir-core' ),
			] );
		}
		...
		function form( $instance ) {
			...
			gulir_create_widget_text_field( [
				...
				'title' => esc_html__( 'Mailchimp Form Shortcode', 'gulir-core' ),
			...
			gulir_create_widget_select_field( [
				...
				'options' => [
					...
					'1' => esc_html__( 'Dark Text', 'gulir-core' ),
				],
			...
			gulir_create_widget_text_field( [
				...
				'title' => esc_html__( 'Background Color (hex value)', 'gulir-core' ),
			...
			gulir_create_widget_text_field( [
				...
				'desc'  => esc_html__( 'Input a background image URL (attachment URL) for this widget.', 'gulir-core' ),
					...
					<?php if ( ! empty( $instance['title'] ) || ! empty( $instance['description'] ) ) : ?>
						<div class="newsletter-content">
							<?php if ( ! empty( $instance['title'] ) ) : ?>
								<span class="h2 newsletter-title"><?php gulir_render_inline_html( $instance['title'] ); ?></span>
							<?php endif;
							if ( ! empty( $instance['description'] ) ) : ?>
								<div class="newsletter-description rb-text"><?php gulir_render_inline_html($instance['description']); ?></div>
							<?php endif; ?>
						</div>
					<?php endif;
