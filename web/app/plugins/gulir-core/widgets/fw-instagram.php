...
if ( ! class_exists( 'Gulir_Fw_Instagram' ) ) {
	class Gulir_Fw_Instagram extends WP_Widget {
		...
		function __construct() {
			...
			$this->params = [
				'header_intro'    => esc_html__( '<span>Follow @ Instagram</span><h6>Our Profile</h6>', 'gulir-core' ),
				'url'             => '#',
				'user_name'       => '',
				'instagram_token' => '',
				'grid_layout'     => 'rb-cmix',
			...
			parent::__construct( $this->widgetID, esc_html__( 'Gulir - Fw Instagram', 'gulir-core' ), [
				'classname'   => $this->widgetID,
				'description' => esc_html__( '[Full Width Widget] Display a grid of instagram images in the full width sections.', 'gulir-core' ),
			] );
		}
		...
		function form( $instance ) {
			...
			gulir_create_widget_textarea_field( [
				...
				'title' => esc_html__( 'Header Intro (raw HTML allowed)', 'gulir-core' ),
			...
			gulir_create_widget_textarea_field( [
				...
				'title'       => esc_html__( 'Instagram Token', 'gulir-core' ),
			...
			gulir_create_widget_select_field( [
				...
				'options' => [
					'rb-cmix'  => esc_html__( 'Wrapper Masonry', 'gulir-core' ),
					'rb-cfmix' => esc_html__( 'Wide Masonry', 'gulir-core' ),
					'rb-grid'  => esc_html__( 'Default Grid', 'gulir-core' ),
				],
			...
			gulir_create_widget_text_field( [
				...
				'title'       => esc_html__( 'Total Images (Default Grid)', 'gulir-core' ),
				'description' => esc_html__( 'This setting will apply only to the default grid layout.', 'gulir-core' ),
			...
			gulir_create_widget_select_field( [
				...
				'title'   => esc_html__( 'Style', 'gulir-core' ),
				'options' => [
					'rb-c5' => esc_html__( '5 columns', 'gulir-core' ),
					'rb-c6' => esc_html__( '6 columns', 'gulir-core' ),
					'rb-c7' => esc_html__( '7 columns', 'gulir-core' ),
					'rb-c8' => esc_html__( '8 columns', 'gulir-core' ),
					'rb-c9' => esc_html__( '9 columns', 'gulir-core' ),
				],
		...
		function widget( $args, $instance ) {
			...
			$data_images = $this->gulir_data_fw_instagram_token( $instance );
				...
				<div class="<?php echo join( ' ', $classes ); ?>">
					<?php if ( ! empty( $instance['header_intro'] ) && ! $flag ) : ?>
						<div class="grid-header">
							<a href="<?php echo esc_url( $instance['url'] ); ?>" target="_blank"><?php echo gulir_strip_tags( $instance['header_intro'] ); ?></a>
						</div>
					<?php endif; ?>
								...
								<div class="grid-el intro-el">
									<div class="instagram-box box-intro">
										...
										<div class="intro-inner">
											...
											<span class="intro-content"><?php gulir_render_inline_html( $instance['header_intro'] ); ?></span>
										</div>
									</div>
								</div>
								<?php $flag = false;
								...
								<div class="instagram-box">
									...
									<div class="box-content">
										<?php if ( ! empty( $image['likes'] ) ) : ?>
											<span class="likes"><i class="rbi rbi-heart" aria-hidden="true"></i><?php gulir_render_inline_html( $image['likes'] ); ?></span>
										<?php endif;
										if ( ! empty( $image['comments'] ) ) : ?>
											<span class="comments"><i class="rbi rbi-comment" aria-hidden="true"></i><?php gulir_render_inline_html( $image['comments'] ); ?></span>
										<?php endif; ?>
									</div>
								</div>
							</div>
		...
		function gulir_data_fw_instagram_token( $settings = [] ) {

			$cache_name  = 'gulir_fw_instagram_cache';
			...
			if ( ! empty( $cache_data[ $cache_id ] ) ) {
				...
				if ( ! empty( $response->data ) && is_array( $response->data ) ) {
					...
					$data_images['error'] = esc_html__( 'Incorrect token or has been expired, Please create a new token and try again!', 'gulir-core' );
				}
