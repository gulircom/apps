...
<div id="bbp-forum-<?php bbp_forum_id(); ?>" <?php bbp_forum_class( 0, 'ruby-single-forum' ); ?>>
    ...
    <div class="bbp-forum-topic-count">
		...
		if ( 1 === $topics || 0 === $topics ) {
			$label = gulir_attr__( 'Topic', 'ruby-bbp' );
		} else {
			$label = gulir_attr__( 'Topics', 'ruby-bbp' );
		} ?>
    ...
    <div class="bbp-forum-reply-count"><?php
		if ( bbp_show_lead_topic() ) :
			...
			if ( 1 === $replies || 0 === $replies ) {
				$replies_label = gulir_attr__( 'Reply', 'ruby-bbp' );
			} else {
				$replies_label = gulir_attr__( 'Replies', 'ruby-bbp' );
			} ?>
		...
		<?php else :
			...
			if ( 1 === $posts || 0 === $posts ) {
				$posts_label = gulir_attr__( 'Post', 'ruby-bbp' );
			} else {
				$posts_label = gulir_attr__( 'Posts', 'ruby-bbp' );
			}
			?>
