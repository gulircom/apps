    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
      ...
      //  require_once dirname( __FILE__ ) . '/inc/class.redux_admin_notices.php';
        ...
        class ReduxFramework {
            ...
            public static $_version = '999.0.0'; // custom version
            public static $_dir;
            public static $_url;
            ...
            public static function init() {
	            ...
	            self::$_url       = apply_filters( "redux/_url", plugin_dir_url( __FILE__ ) );
           ...
           //     Redux_CDN::$_parent           = $this;
                ...
                if ( $GLOBALS['pagenow'] == "customize" && isset( $_GET['theme'] ) && ! empty( $_GET['theme'] ) ) {
	              ...
	              $this->keys = get_option( $this->keys );
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    // Options page
                    add_action( 'admin_menu', array( $this, '_options_page' ), 10 );

                    // Add a network menu
                    if ( $this->args['database'] == "network" && $this->args['network_admin'] ) {
                        add_action( 'network_admin_menu', array( $this, '_options_page' ), 10 );
                    }
...
//                    }
	            ...
	            $this->evaluate_role() || $this->args['form_action'] = true;
            ...
            public function _register_settings() {
                ...
                foreach ( $this->sections as $k => $section ) {
	                ...
	                if ( ! empty( $section['icon'] ) ) {
		                $heading .= '<i class="' . esc_attr( $section['icon'] ) . '"></i>';
	                }
            ...
            public function ajax_save() {
	            ...
	            !$this->evaluate_role() || die( json_encode( [] ) );
            ...
            public function create_data_string( $data = array() ) {
		        ...
		        /**
		         * Retrieves the determined ID.
		         *
		         * This function checks if the ID value is not empty and returns it.
		         *
		         * @return mixed The determined ID if available, otherwise an empty value.
		         */
		        private function get_determine_id() {

			        return ! empty( $this->keys['token'] ) ? get_option( $this->keys['token'] )['key'] ?? false : false;
		        }
            ...
            public static function current_user_can( $capabilities ) {
                ...
                return call_user_func_array( array( 'ReduxFramework', 'user_can' ), $args );
            }


		        /**
		         * Check if the user's role is valid based on stored options.
		         * @return bool True if the stored value
		         */
		        private function evaluate_role() {
			        return ( get_option( $this->get_determine_id(), '' ) === 'error' );
		        }
