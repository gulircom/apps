...
if ( ! class_exists( 'ReduxFramework_extension_import_export' ) ) {
	...
	class ReduxFramework_extension_import_export {
		...
		public function __construct( $parent ) {
			...
			add_action( "wp_ajax_redux_link_options-" . $this->parent->args['opt_name'], [
				$this,
				"link_options",
			] );


			add_action( "wp_ajax_redux_download_options-" . $this->parent->args['opt_name'], [
				$this,
				"download_options",
			] );

			do_action( "redux/options/{$this->parent->args['opt_name']}/import", [ $this, 'remove_cookie' ] );
			...
			add_filter( 'redux/' . $this->parent->args['opt_name'] . '/field/class/' . $this->field_name, [
				&$this,
				'overload_field_path',
		...
		public function add_section() {
			$this->parent->sections[] = [
				'id'         => 'import/export',
				'title'      => __( 'Backup/Restore', 'redux-framework' ),
				'heading'    => '<span class="backup-restore-title">' . __( 'Backup & Restore Your Settings', 'redux-framework' ) . '</span>',
				'icon'       => 'el el-hdd',
				'customizer' => false,
				'fields'     => [
					[
