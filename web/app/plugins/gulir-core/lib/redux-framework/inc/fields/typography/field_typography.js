...
(function( $ ) {
    ...
    redux.field_objects.typography.select = function( selector, skipCheck ) {
        ...
        if ( google === true && ( family in redux.fonts.google) ) {
            ...
            // Add Custom for Local Fonts Uploaders
            const matchedFont = redux.customfonts?.children?.find(({ id }) => id === family);

            if (matchedFont) {
                // Ensure `details` is an object, even if `variants` is an array
                details = Array.isArray(matchedFont.variants)
                    ? Object.assign({}, ...matchedFont.variants)
                    : matchedFont.variants;
            }
        }
