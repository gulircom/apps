        ...
        class ReduxFramework_section {
            ...
            public function render() {
	            ...
	            if ( ! empty( $this->field['subtitle'] ) ) {
		            if ( is_array( $this->field['subtitle'] ) ) {
			            foreach ( $this->field['subtitle'] as $subtitle ) {
				            echo '<div class="redux-section-desc">' . esc_html( $subtitle ) . '</div>';
			            }
		            } else {
			            echo '<div class="redux-section-desc">' . esc_html( $this->field['subtitle'] ) . '</div>';
		            }
	            } elseif ( ! empty( $this->field['notice'] ) && is_array( $this->field['notice'] ) ) {
		            foreach ( $this->field['notice'] as $subtitle ) {
			            echo '<div class="redux-section-desc">' . esc_html( $subtitle ) . '</div>';
		            }
	            }

                echo '</div><table id="section-table-' . esc_attr($this->field['id']) . '" data-id="' . esc_attr($this->field['id']) . '" class="form-table form-table-section ' . esc_attr($add_class) . '"><tbody><tr class="no-border"><th></th><td id="' . esc_attr($guid) . '">';
