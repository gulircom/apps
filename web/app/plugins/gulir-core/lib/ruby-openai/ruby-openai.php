...
if ( ! class_exists( 'RB_OPENAI_ASSISTANT', false ) ) {
	class RB_OPENAI_ASSISTANT {
		...
		private static $parent_slug = 'gulir-admin';
		public $menu_id;
		...
		public function add_admin_menu() {
			...
			$this->menu_id = add_submenu_page(
				self::$parent_slug,
				esc_html__( 'OpenAI Assistant', 'gulir-core' ),
				esc_html__( 'OpenAI Assistant', 'gulir-core' ),
				$this->capability,
				'ruby-openai',
		...
		function save() {
			...
			if ( ! check_ajax_referer( 'rb-openai', '_wpnonce_ruby' ) ) {
				wp_send_json_error( esc_html__( 'Invalid nonce verification', 'gulir-core' ), 400 );

				wp_die();
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_send_json_error( esc_html__( 'Sorry, you are not allowed to perform this action.', 'gulir-core' ), 403 );
				...
				<div class="rb-dashboard-section rb-dashboard-fw">
					<div class="rb-intro-content">
						<div class="rb-intro-content-inner">
							<h2 class="rb-dashboard-title">
								<?php esc_html_e( 'Ruby OpenAI Assistant', 'gulir-core' ); ?>
							</h2>
							<p class="rb-dashboard-tagline"><?php esc_html_e( 'Powered by the cutting-edge OpenAI technology (GTP 3.5 Turbo), Ruby OpenAI Assistant tool is designed to make content creation an effortless and inspiring experience.', 'gulir-core' ); ?></p>
						</div>
					...
					<div class="rb-dashboard-steps col-50">
						...
						<a class="<?php echo esc_attr( $step_class ); ?>" href="https://platform.openai.com/api-keys" target="_blank" rel="nofollow">
							<h3><?php esc_html_e( 'Create Your OpenAI API Key', 'gulir-core' ); ?></h3>
							<p class="rb-dashboard-desc"><?php esc_html_e( 'Generate your own API key through your OpenAI account to access the API features.', 'gulir-core' ); ?></p>
							<div class="rb-step-icon"><i class="rbi-dash rbi-dash-key"></i></div>
						</a>
						<a class="rb-dashboard-step" href="<?php echo admin_url( 'post-new.php#rb_meta_gulir_post_options' ); ?>" target="_blank">
							<h3><?php esc_html_e( 'Generate Ideas for Your Posts', 'gulir-core' ); ?></h3>
							<p class="rb-dashboard-desc"><?php esc_html_e( 'By providing a text prompt, the API will generate a text completion that aligns with the pattern you specified.', 'gulir-core' ); ?></p>
					...
					<form id="rb-openai" name="rb-openai" method="post" action="">
						...
						<div class="rb-form-wrap is-boxed">
							<div class="rb-panel-inline">
								<span class="rb-panel-label"><?php esc_html_e( 'OpenAI Assistant', 'gulir-core' ); ?></span>
							...
							<div class="rb-panel-inline">
								<label class="rb-panel-inline-label" for="api-key"><?php esc_html_e( 'OpenAI API Key', 'gulir-core' ); ?>
									<span class="rb-form-tip"><i class="rbi-dash rbi-dash-info"></i><span class="rb-form-tip-content"><?php
											...
											printf( __( 'Generate your API key by following this <a href=%1$s target="_blank" >link</a>', 'gulir-core' ), '//platform.openai.com/account/api-keys' );
											?></span></span>
								</label>
								<input id="api-key" type="text" name="rb_openai_api_key" placeholder="sk-xxxx..........................." value="<?php echo esc_attr( $rb_openai_api_key ); ?>">
							</div>
							<div class="rb-panel-inline">
								<label class="rb-panel-inline-label" for="max-tokens"><?php esc_html_e( 'Max Token', 'gulir-core' ); ?>
									<span class="rb-form-tip"><i class="rbi-dash rbi-dash-info"></i><span class="rb-form-tip-content"><?php esc_html_e( 'Maximum number of tokens in the generated output. It controls the length of the generated text. ', 'gulir-core' ); ?></span></span>
								</label>
								<input id="max-tokens" type="text" name="rb_openai_max_tokens" placeholder="200" value="<?php echo esc_attr( $rb_openai_max_tokens ); ?>">
							</div>
							<div class="rb-panel-inline">
								<label class="rb-panel-inline-label" for="temperature"><?php esc_html_e( 'Temperature', 'gulir-core' ); ?>
									<span class="rb-form-tip"><i class="rbi-dash rbi-dash-info"></i><span class="rb-form-tip-content"><?php esc_html_e( 'Controls the randomness of the output. Higher values (e.g., 1.2) make the output more random, while lower values (e.g., 0.8) make it more focused and deterministic.', 'gulir-core' ); ?></span></span>
								</label>
								<input id="temperature" type="text" name="rb_openai_temperature" placeholder="0.8" value="<?php echo esc_attr( $rb_openai_temperature ); ?>">
							</div>
							<div class="rb-panel-inline">
								<label class="rb-panel-inline-label" for="writing-style"><?php esc_html_e( 'Writing Style', 'gulir-core' ); ?>
									<span class="rb-form-tip"><i class="rbi-dash rbi-dash-info"></i><span class="rb-form-tip-content"><?php esc_html_e( 'Choose a writing style that aligns with the type of content you intend to publish and resonates with your target audience.', 'gulir-core' ); ?></span></span>
								</label>
							...
							<div class="rb-panel-inline no-border">
								<label class="rb-panel-inline-label" for="writing-style"><?php esc_html_e( 'Writing Language', 'gulir-core' ); ?>
									<span class="rb-form-tip"><i class="rbi-dash rbi-dash-info"></i><span class="rb-form-tip-content"><?php esc_html_e( 'Choose a writing language to create content.', 'gulir-core' ); ?></span></span>
								</label>
						...
						<div class="rb-form-sticky">
							...
							<button type="submit" name="action" class="rb-panel-button is-big-button" id="rb-submit-api" value="update">
								...
								<?php echo esc_html__( 'Save Changes', 'gulir-core' ); ?></button>
						</div>
					</form>
					<div></div>
				</div>
		...
		public function meta_configs( $configs ) {
			...
			$configs['tabs'][] = [
				...
				'fields' => [
					[
						'id'       => 'ai_assistant',
						'name'     => esc_html__( 'OpenAI Assistant', 'gulir-core' ),
						'type'     => 'html_template',
		...
		public function create_content() {
			...
			if ( ! $prompt ) {
				wp_send_json_error( esc_html__( 'Invalid prompt', 'gulir-core' ), 403 );
				wp_die();
			}
