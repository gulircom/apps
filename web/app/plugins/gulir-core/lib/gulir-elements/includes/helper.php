...
if ( ! function_exists( 'gulir_get_block_attributes' ) ) {
	function gulir_get_block_attributes( $block_name, $post_id, $client_id ) {
...
if ( ! function_exists( 'gulir_get_block_padding_css' ) ) {
	function gulir_get_block_padding_css( $padding = [] ) {
...
if ( ! function_exists( 'gulir_get_block_border_width_css' ) ) {
	function gulir_get_block_border_width_css( $border = [] ) {
...
if ( ! function_exists( 'gulir_block_review_schema_markup' ) ) {
	function gulir_block_review_schema_markup( $attributes ) {
		...
		$item['average'] = gulir_block_review_get_average( $attributes );
		...
		if ( ! empty( $attributes['productCons'] ) ) {
			$item['cons'] = $attributes['productCons'];
		}

		if ( ! empty( $attributes['buyButtons'] ) ) {
			$item['offers'] = $attributes['buyButtons'];
			if ( ! empty( $attributes['salePrice'] ) ) {
				$item['price'] = gulir_get_price_number( $attributes['salePrice'] );
			} elseif ( ! empty( $attributes['price'] ) ) {
				$item['price'] = gulir_get_price_number( $attributes['price'] );
			}
		...
		gulir_review_product_schema_markup( $item );
	}
}

if ( ! function_exists( 'gulir_get_price_number' ) ) {
	function gulir_get_price_number( $price ) {
...
if ( ! function_exists( 'gulir_review_product_schema_markup' ) ) {
	function gulir_review_product_schema_markup( $item ) {
