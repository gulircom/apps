...
add_action( 'save_post', 'gulir_set_main_block_review', 20 );

if ( ! function_exists( 'gulir_render_block_review' ) ) {
	function gulir_render_block_review( $attributes ) {
		...
		$avg_value = gulir_block_review_get_average( $attributes );

		$output .= '<div ' . get_block_wrapper_attributes( [
						'class' => $wrapperClassName,
						'style' => gulir_get_block_review_style( $attributes ),
				] ) . '>';
		...
		if ( ! empty( $attributes['overlayLink'] ) ) {
			...
			if ( ! $overlayLinkInternal ) {
				$overlayLinkRel .= 'nofollow noopener';
		...
		if ( ! empty( $attributes['shadow'] ) ) {
			$output .= gulir_block_review_featured_image( $attributes );
		}
		...
		if ( $avg_value && function_exists( 'gulir_get_review_stars' ) ) {
			$output .= '<div class="review-total-stars">';

			if ( function_exists( 'gulir_html__' ) ) {
				$out_of_label = gulir_html__( 'out of 5', 'gulir-core' );
			} else {
				$out_of_label = esc_html__( 'out of 5', 'gulir-core' );
			}

			$output .= gulir_get_review_stars( $avg_value );
		...
		if ( ! empty( $attributes['description'] ) ) {
			$output .= '<div class="gb-review-description top-divider"><div class="gb-description rb-text">' . gulir_strip_tags( $attributes['description'] ) . '</div></div>';
		}
		$output .= gulir_block_review_features( $attributes );
		$output .= gulir_block_review_pros_cons( $attributes );
		$output .= gulir_block_review_buttons( $attributes );
		$output .= '</div>';
		$output .= '</div>';
		...
		add_action(
				...
				function () use ( $attributes ) {
					gulir_block_review_schema_markup( $attributes );
				}, 5
		);
...
if ( ! function_exists( 'gulir_get_block_review_style' ) ) {
	function gulir_get_block_review_style( $attributes ) {
...
if ( ! function_exists( 'gulir_block_review_featured_image' ) ) {

	function gulir_block_review_featured_image( $attributes ) {
		...
		if ( ! empty( $attributes['image'] ) ) {
			$size   = gulir_get_image_size( $attributes['image'] );
			...
			if ( ! empty( $attributes['meta'] ) ) {
				...
				$output .= gulir_block_review_get_average( $attributes );
				$output .= '</span>';
...
if ( ! function_exists( 'gulir_block_review_get_average' ) ) {

	function gulir_block_review_get_average( $attributes ) {
...
if ( ! function_exists( 'gulir_block_review_pros_cons' ) ) {
	function gulir_block_review_pros_cons( $attributes ) {
...
if ( ! function_exists( 'gulir_block_review_features' ) ) {
	function gulir_block_review_features( $attributes ) {
			...
			<?php foreach ( $attributes['productFeatures'] as $element ) :
				...
				<div class="review-el">
					<div class="review-label">
						...
						<span class="rating-info is-meta"><?php printf( gulir_html__( '%s out of 5', 'gulir-core' ), $element['rating'] ); ?></span>
					</div>
					<span class="review-rating"><?php echo gulir_get_review_stars( $element['rating'] ); ?></span>
				</div>
			<?php endforeach;
			?>
		</div>
...
if ( ! function_exists( 'gulir_block_review_buttons' ) ) {
	function gulir_block_review_buttons( $attributes ) {
		...
		$output .= '<div class="gb-buttons top-divider">';
		foreach ( $attributes['buyButtons'] as $item ) {
			...
			$output      .= '<a class="is-btn gb-btn' . ( ! empty( $item['isButtonBorder'] ) ? ' btn-outlined' : '' ) . '"';
			$output      .= 'href="' . $link . '" target="_blank" rel="nofollow noopener' . ( $isSponsored ? ' sponsored' : '' ) . '">' . esc_html( $item['label'] ) . '</a>';
		}
		$output .= '</div>';
...
if ( ! function_exists( 'gulir_set_main_block_review' ) ) {
	/**
	 * @param $post_id
	 */
	function gulir_set_main_block_review( $post_id ) {
		...
		if ( function_exists( 'gulir_is_review_post' ) && gulir_is_review_post( $post_id ) ) {
			return;
		}

		/** remove if empty data */
		delete_post_meta( $post_id, 'gulir_review_average' );
		delete_post_meta( $post_id, 'gulir_block_review_metadata' );

		// Check if the post type supports blocks.
		$post_content     = get_post_field( 'post_content', $post_id );
		$block_attributes = gulir_extract_first_review_block( $post_content );
		if ( empty( $block_attributes ) ) {
			$block_attributes = gulir_extract_first_review_reusable( $post_content );
		}
		...
		$avg_value = gulir_block_review_get_average( $block_attributes );
		update_post_meta( $post_id, 'gulir_block_review_metadata',
				[
						'type'    => 'star',
		...
		update_post_meta( $post_id, 'gulir_review_average', floatval( $avg_value ) * 2 );
	}
}

if ( ! function_exists( 'gulir_extract_first_review_block' ) ) {
	/**
	 * @param $content
	 *
	 * @return array|mixed
	 */
	function gulir_extract_first_review_block( $content ) {

		$pattern = '/<!--\s*wp:gulir-elements\/review\s*(.*?)\s*\/-->/s';
		$matches = [];
...
if ( ! function_exists( 'gulir_extract_first_review_reusable' ) ) {
	/**
	 * @param $content
	 *
	 * @return array|mixed
	 */
	function gulir_extract_first_review_reusable( $content ) {
