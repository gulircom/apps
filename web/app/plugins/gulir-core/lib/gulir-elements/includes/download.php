...
add_action( 'wp_ajax_form_download_submit', 'gulir_form_download_submit' );
add_action( 'wp_ajax_nopriv_form_download_submit', 'gulir_form_download_submit' );

if ( ! function_exists( 'gulir_email_to_download' ) ) {
	function gulir_email_to_download( $attributes ) {
		...
		$output .= '<div ' . get_block_wrapper_attributes( [
				...
				'style' => gulir_get_block_download_style( $attributes ),
			] ) . '>';
		...
		if ( ! empty( $description ) ) {
			$output .= '<div class="gb-description rb-text">' . gulir_strip_tags( $description ) . '</div>';
		}
		$output       .= '</div>';
		...
		if ( ! empty( $integrations['gulir']['double_optin'] ) ) {
...
if ( ! function_exists( 'gulir_get_block_download_style' ) ) {
	function gulir_get_block_download_style( $attributes ) {
...
if ( ! function_exists( 'gulir_form_download_submit' ) ) {
	function gulir_form_download_submit() {
		...
		if ( empty( $email ) ) {
			wp_send_json( [
				'success' => false,
				'message' => esc_html__( 'Something went wrong, please try again!', 'gulir-core' ),
			] );
		}

		/** add */
		do_action( 'gulir_subscribe' );
		...
		$data = gulir_get_block_attributes( 'gulir-elements/download', $post_id, $block_id );

		if ( function_exists( 'gulir_html__' ) ) {
			$success_message = gulir_html__( 'Your download will start in a few seconds, If your download does not start, please click here.', 'gulir-core' );
			$error_message   = gulir_html__( 'Sorry, File not found.', 'gulir-core' );
		} else {
			$success_message = esc_html__( 'Your download will start in a few seconds, If your download does not start, please click here.', 'gulir-core' );
			$error_message   = esc_html__( 'Sorry, File not found.', 'gulir-core' );
		}
