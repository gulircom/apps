...
add_action( 'plugins_loaded', [ 'Gulir_Membership', 'get_instance' ], 0 );

if ( ! class_exists( 'Gulir_Membership', false ) ) {
	class Gulir_Membership {
		...
		public function __construct() {
			...
			add_filter( 'gulir_entry_title_classes', [ $this, 'add_classes' ], 10, 2 );
		}
		...
		function enqueue_style() {
			...
			wp_enqueue_style( 'gulir-swpm', GULIR_CORE_URL . 'assets/' . $path, [], GULIR_CORE_VERSION, 'all' );
		}
		...
		public function restrict_box( $error_msg ) {
			...
			$restrict_title = gulir_get_option( 'restrict_title' );
			$restrict_desc  = gulir_get_option( 'restrict_desc' );
			$join_us_label  = gulir_get_option( 'join_us_label' );
			$login_label    = gulir_get_option( 'login_label' );
			$login_desc     = gulir_get_option( 'login_desc' );

			$output = '';
			$output .= '<div class="restrict-box"><div class="restrict-box-inner">';

			$output .= '<h2 class="restrict-title">' . gulir_strip_tags( $restrict_title ) . '</h2>';
			$output .= '<p class="restrict-desc">' . gulir_strip_tags( $restrict_desc ) . '</p>';
			...
			$output .= '<span class="restrict-login-description">' . gulir_strip_tags( $login_desc ) . '</span>';
		...
		public function restrict_level_box( $error_msg ) {
			...
			$restrict_title = gulir_get_option( 'restrict_level_title' );
			$restrict_desc  = gulir_get_option( 'restrict_level_desc' );
			$join_us_label  = gulir_get_option( 'join_us_label' );
		...
		public function restrict_renewal_box( $error_msg ) {
			...
			$restrict_title = gulir_get_option( 'restrict_renewal_title' );
			$restrict_desc  = gulir_get_option( 'restrict_renewal_desc' );
			$renewal_label  = gulir_get_option( 'renewal_label' );
