...
if ( ! function_exists( 'rb_weather_data' ) ) {
	...
	function rb_weather_data( $options ) {
		...
		$units_display       = ( $units === "metric" ) ? esc_html__( 'C', 'gulir-core' ) : esc_html__( 'F', 'gulir-core' );
		$locale              = 'en';
		$now_w_icon          = '';
		$ruby_today_temp     = '';
		$name_country        = '';
		...
		if ( in_array( $system_locale, $gulirlocales ) ) {
			$locale = $system_locale;
		}

		if ( in_array( substr( $system_locale, 0, 2 ), $gulirlocales ) ) {
		...
		$weather_transient_name = 'gulir_' . $city_name_slug . "_" . strtolower( $units_display ) . '_' . $locale;
		...
		if ( get_transient( $weather_transient_name ) ) {
			...
			if ( is_wp_error( $now_ping_get ) ) {
				return gulir_get_weather_error( $weather_transient_name );
			}
			...
			if ( $days_to_show !== 'hide' ) {
				...
				$rb_forecast_ping_get = wp_remote_get( $rb_forecast_ping, [ 'timeout' => 30 ] );
		...
		$speed_text = ( $units === "metric" ) ? esc_html__( 'km/h', 'gulir-core' ) : esc_html__( 'mph', 'gulir-core' );
		...
		if ( ! empty( $ruby_today->main->humidity ) ) {
			$weather_humidity = $ruby_today->main->humidity . '' . esc_html__( '%', 'gulir-core' );
		}
			...
			<div class="<?php echo strip_tags( $classes ); ?>">
				<?php if ( ! empty( $title ) ) : ?>
					<div class="rb-w-title h4">
						<?php gulir_render_inline_html( $title ) ?>
					</div>
				<?php endif; ?>
				<div class="rb-w-header">
					<div class="col-left">
						<div class="rb-w-big-icon">
							<?php echo Gulir_SVG_Icons::get_weather_icon( $now_w_icon, $now_w_icon . '-current' ); ?>
						</div>
					</div>
					<div class="col-right">
						<div class="rb-w-units h6">
							<span><?php echo strip_tags( $ruby_today_temp ) ?></span><span class="ruby-degrees"><sup><?php echo '&deg;' . gulir_strip_tags( $units_display ) ?></sup></span>
						</div>
					</div>
				</div>
					...
					<div class="w-forecast-wrap">
						...
						foreach ( $rb_forecast_days as $rb_forecast_day ) :
							...
							<div class="w-forecast-day forecast-day-<?php echo strip_tags( $days_to_show ) ?>">
								...
								<div class="w-forecast-icon"><?php echo Gulir_SVG_Icons::get_weather_icon( $forecast_icon, $forecast_icon . '-' . $count ) ?></div>
...
if ( ! function_exists( 'gulir_get_weather_error' ) ) {
	function gulir_get_weather_error( $weather_transient_name = '' ) {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			...
			$error_message .= esc_html__( 'No weather information available. Please check your location here: ', 'gulir-core' );
			$error_message .= '<a target="_blank" href="https://openweathermap.org/find/">';
			$error_message .= esc_html__( 'Find location', 'gulir-core' );
			$error_message .= '</a></div>';
			...
			<div class="banner-bg">
				<?php if ( ! empty( $instance['e_image']['id'] ) ) :
					...
					if ( ! empty( $instance['e_dark_image']['id'] ) ) :
						...
						<img loading="lazy" decoding="async" data-mode="default" src="<?php echo esc_url( $image[0] ); ?>" alt="<?php echo esc_attr__( 'banner', 'gulir-core' ); ?>" width="<?php echo strip_tags( $image[1] ); ?>" height="<?php echo strip_tags( $image[2] ); ?>"/>
						<img loading="lazy" decoding="async" data-mode="dark" src="<?php echo esc_url( $dark_image[0] ); ?>" alt="<?php echo esc_attr__( 'banner', 'gulir-core' ); ?>" width="<?php echo strip_tags( $dark_image[1] ); ?>" height="<?php echo strip_tags( $dark_image[2] ); ?>"/>
					<?php else : ?>
						<img loading="lazy" decoding="async" src="<?php echo esc_url( $image[0] ); ?>" alt="<?php echo esc_attr__( 'banner', 'gulir-core' ); ?>" width="<?php echo strip_tags( $image[1] ); ?>" height="<?php echo strip_tags( $image[2] ); ?>"/>
					<?php endif;
				else :
					if ( ! empty( $instance['image'] ) ) :
						$image_size = gulir_get_image_size( $instance['image'] );
						if ( ! empty( $instance['dark_image'] ) ) :
							$dark_image_size = gulir_get_image_size( $instance['dark_image'] );
							?>
							<img loading="lazy" decoding="async" data-mode="default" src="<?php echo esc_url( $instance['image'] ); ?>" alt="<?php echo esc_attr__( 'banner', 'gulir-core' );
							?>" width="<?php if ( ! empty( $image_size[0] ) ) {
								echo strip_tags( $image_size[0] );
							} ?>" height="<?php if ( ! empty( $image_size[1] ) ) {
								echo strip_tags( $image_size[1] );
							} ?>">
							<img loading="lazy" decoding="async" data-mode="dark" src="<?php echo esc_url( $instance['dark_image'] ); ?>" alt="<?php echo esc_attr__( 'banner', 'gulir-core' );
						...
						<?php else : ?>
							<img loading="lazy" decoding="async" src="<?php echo esc_url( $instance['image'] ); ?>" alt="<?php echo esc_attr__( 'banner', 'gulir-core' );
							?>" width="<?php if ( ! empty( $image_size[0] ) ) {
								echo strip_tags( $image_size[0] );
							} ?>" height="<?php if ( ! empty( $image_size[1] ) ) {
								echo strip_tags( $image_size[1] );
			...
			<div class="<?php echo strip_tags( $inner_classes ); ?>">
				<div class="content-inner">
					<?php if ( ! empty( $instance['title'] ) ) : ?>
						<h5 class="w-banner-title h2"><?php gulir_render_inline_html( $instance['title'] ); ?></h5>
					<?php endif;
					if ( ! empty( $instance['description'] ) ) : ?>
						<div class="w-banner-desc element-desc"><?php gulir_render_inline_html( $instance['description'] ); ?></div>
					<?php endif;
					if ( ! empty( $instance['url'] ) ) : ?>
						<a class="banner-btn is-btn" href="<?php echo esc_url( $instance['url'] ) ?>" target="_blank" rel="noopener nofollow"><?php gulir_render_inline_html( $instance['submit'] ) ?></a>
					<?php endif; ?>
				</div>
			</div>
		</div>
...
if ( ! function_exists( 'gulir_render_covid_data' ) ) {
	/**
	 * @param $attrs
	 *
	 * @return false|string
	 */
	function gulir_render_covid_data( $attrs ) {
		...
		$settings = shortcode_atts( [
			...
			'title_tag'       => '',
			'icon'            => '',
			'confirmed_label' => gulir_html__( 'Confirmed', 'gulir-core' ),
			'death_label'     => gulir_html__( 'Death', 'gulir-core' ),
			'confirmed'       => '',
			'deaths'          => '',
			'date'            => '1',
		], $attrs );
		...
		<div class="block-covid-data">
			<div class="data-inner">
				<?php if ( ! empty( $settings['country_name'] ) ) {
					echo '<div class="country-name"><' . strip_tags( $settings['title_tag'] ) . '>' . gulir_strip_tags( $settings['country_name'] ) . '</' . strip_tags( $settings['title_tag'] ) . '></div>';
				} ?>
				<div class="data-item data-confirmed">
					<p class="description-text">
						<span class="data-item-icon"><?php gulir_render_svg( 'chart' ); ?></span><?php gulir_render_inline_html( $settings['confirmed_label'] ); ?>
					</p>
					<p class="data-item-value h5"><?php echo gulir_pretty_number( $data['confirmed'] ); ?></p>
				</div>
				<div class="data-item data-death">
					<p class="description-text">
						<span class="data-item-icon"><?php gulir_render_svg( 'chart' ); ?></span><?php gulir_render_inline_html( $settings['death_label'] ); ?>
					</p>
					<p class="data-item-value h5"><?php echo gulir_pretty_number( $data['deaths'] ); ?></p>
				</div>
				<?php if ( ! empty( $settings['icon'] ) ) {
					gulir_render_svg( 'virus' );
				} ?>
			</div>
		</div>
...
if ( ! function_exists( 'gulir_render_pricing_plan' ) ) {
	 ...
	 * @return string
	 */
	function gulir_render_pricing_plan( $settings ) {
		...
		if ( ! empty( $settings['title'] ) ) {
			$output .= '<h2 class="plan-heading">' . gulir_strip_tags( $settings['title'] ) . '</h2>';
		}
		if ( ! empty( $settings['description'] ) ) {
			$output .= '<p class="plan-description">' . gulir_strip_tags( $settings['description'] ) . '</p>';
		}
		$output .= '</div>';
		...
		if ( ! empty( $settings['price'] ) ) {
			...
			if ( ! empty( $settings['unit'] ) ) {
				$output .= '<span class="plan-price-unit">' . gulir_strip_tags( $settings['unit'] ) . '</span>';
			}
			$output .= '<span class="plan-price">' . gulir_strip_tags( $settings['price'] ) . '</span>';
			if ( ! empty( $settings['tenure'] ) ) {
				$output .= '<span class="plan-tenure">' . gulir_strip_tags( $settings['tenure'] ) . '</span>';
			}
			$output .= '</div>';
		}
		...
		if ( is_array( $settings['features'] ) ) {
			...
			foreach ( $settings['features'] as $feature ) {
				if ( ! empty( $feature['feature'] ) ) {
					$output .= '<span class="plan-feature">' . gulir_strip_tags( $feature['feature'] ) . '</span>';
				}
			}
			$output .= '</div>';
		}
		...
		if ( ! empty( $settings['shortcode'] ) ) {
			...
			$output .= '<a class="button" href="' . SwpmSettings::get_instance()->get_value( 'registration-page-url' ) . '">' . gulir_strip_tags( $settings['register_button'] ) . '</a>';
		}
		$output .= '</div>';
