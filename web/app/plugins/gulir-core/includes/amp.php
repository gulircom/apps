...
add_action( 'plugins_loaded', [ 'Gulir_AMP', 'get_instance' ], 3 );

if ( ! class_exists( 'Gulir_AMP', false ) ) {
	class Gulir_AMP {
		...
		public function print_mode() {
			...
			if ( $current_screen->id === 'toplevel_page_amp-options' && defined( 'AMP__VERSION' ) && AMP__VERSION >= 2 ) : ?>
                    ...
                    (function ($) {
                        var templateModeTimeOut = setInterval(function () {
                            ...
                            if (templateModes.length > 0) {
                                templateModes.html('<div class="selectable selectable--left" style="box-shadow: -10px 0 0 #6cc296;"><div class="settings-welcome__illustration"><h3><?php esc_html_e( 'Gulir supported and activated AMP in the Transitional mode.', 'gulir-core' ); ?></h3></div></div>');
				...
				<script>
                    (function ($) {
                        $('.amp-website-mode').find('td').html('<p class="notice notice-success"><?php esc_html_e( 'Gulir supported and activated AMP in the Transitional mode.', 'gulir-core' ); ?></p>');
