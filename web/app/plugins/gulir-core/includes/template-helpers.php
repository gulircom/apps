...
if ( ! function_exists( 'gulir_get_breadcrumb' ) ) {
	function gulir_get_breadcrumb( $classes = '', $check_setting = true ) {
		...
		if ( $check_setting && ! gulir_get_option( 'breadcrumb' ) ) {
			return false;
		}

		ob_start();
		$class_name = 'breadcrumb-wrap';
		if ( gulir_get_option( 'breadcrumb_style' ) ) {
		...
		if ( function_exists( 'bcn_display' ) ) :
			...
			<aside class="<?php echo esc_attr( $class_name ); ?>">
				<div class="breadcrumb-inner" vocab="<?php echo gulir_protocol(); ?>://schema.org/" typeof="BreadcrumbList"><?php bcn_display(); ?></div>
			</aside>
		<?php
		else :
...
if ( ! function_exists( 'gulir_get_wp_errors' ) ) {
	function gulir_get_wp_errors( $errors ) {
