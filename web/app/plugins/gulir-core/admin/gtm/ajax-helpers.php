...
if ( ! class_exists( 'Ruby_Ajax_Google_Tags' ) ) {
	class Ruby_Ajax_Google_Tags {
		...
		function remove() {
			...
			wp_send_json_success( esc_html__( 'Tags successfully removed.', 'gulir-core' ));
		...
		function add() {
			...
			if ( empty( $gtm_id ) && empty( $gtag_id ) ) {
				wp_send_json_error( esc_html__( 'Tags not found!', 'gulir-core' ), 403 );
			...
			wp_send_json_success( esc_html__( 'Google Tag has been successfully added.', 'gulir-core' ) );
