...
class WXR_Parser_Regex {
	...
	function parse( $file ) {
		...
		if ( defined( 'RUBY_API_HOST' ) ) {
			...
			$user_agent = '';
			$host_name  = parse_url( get_bloginfo( 'url' ) );
			if ( ! empty( $host_name['host'] ) ) {
				$user_agent = 'WordPress/' . get_bloginfo( 'version' ) . '; ' . $host_name['host'];
			}
			$response = wp_remote_get( trailingslashit( RUBY_API_HOST ) . $file, [
			...
			if ( ! is_wp_error( $response ) ) {
				...
				if ( $body ) {
					...
					$file       = tempnam( $upload_dir['path'], 'tempfile_' );
					$fp         = fopen( $file, 'w' );
					if ( $fp ) {
						// Write the remote content to the temporary file
						fwrite( $fp, $body );
						fclose( $fp );
					}
				}
