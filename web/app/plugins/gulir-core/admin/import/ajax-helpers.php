	...
	class Ruby_Importer {
		...
		public function cleanup_content() {
			...
			if ( empty( $nonce ) || false === wp_verify_nonce( $nonce, self::$nonce ) ) {
				wp_send_json_error( esc_html__( 'Nonce validation failed. Please try again.', 'gulir-core' ), 400 );
			...
			if ( empty( $content ) ) {
				wp_send_json_error( esc_html__( 'Configuration is incorrect.', 'gulir-core' ), 400 );
		...
		public function init() {
			...
			if ( ! isset( $demos[ $directory ] ) ) {
				wp_send_json_error( esc_html__( 'Demo name not found.', 'gulir-core' ), 400 );
		...
		function delete_imported_content() {
			...
			self::save_import_progress( esc_html__( 'Purge dummy content...', 'gulir-core' ), 5 );
		...
		function after_import_content( $directory = '' ) {
			...
			$demos = $this->get_demos();
			self::save_import_progress( esc_html__( 'Finishing...', 'gulir-core' ), 0 );
			...
			if ( isset( $main_menu->term_id ) ) {
				$menu_locations['gulir_main'] = $main_menu->term_id;
			}
			if ( isset( $mobile_menu->term_id ) ) {
				$menu_locations['gulir_mobile'] = $mobile_menu->term_id;
			}
			if ( isset( $quick_menu->term_id ) ) {
				$menu_locations['gulir_mobile_quick'] = $quick_menu->term_id;
			}
		...
		function register_demo_widgets() {
			...
			/** empty sidebars */
			$sidebars_widgets['gulir_sidebar_default']          = [];
			$sidebars_widgets['gulir_sidebar_more']             = [];
			$sidebars_widgets['gulir_sidebar_fw_footer']        = [];
			$sidebars_widgets['gulir_sidebar_footer_1']         = [];
			$sidebars_widgets['gulir_sidebar_footer_2']         = [];
			$sidebars_widgets['gulir_sidebar_footer_3']         = [];
			$sidebars_widgets['gulir_sidebar_footer_4']         = [];
			$sidebars_widgets['gulir_sidebar_footer_5']         = [];
			$sidebars_widgets['gulir_entry_top']                = [];
			$sidebars_widgets['gulir_entry_bottom']             = [];
			$sidebars_widgets['gulir_sidebar_multi_sb1']        = [];
			$sidebars_widgets['gulir_sidebar_multi_sb2']        = [];
			$sidebars_widgets['gulir_sidebar_multi_next-posts'] = [];
			$sidebars_widgets['gulir_sidebar_multi_single']     = [];
			$sidebars_widgets['gulir_sidebar_multi_blog']       = [];
			$sidebars_widgets['gulir_sidebar_multi_contact']    = [];

			/** add sidebars */
			$theme_options                   = get_option( GULIR_TOS_ID, [] );
			$theme_options['multi_sidebars'] = [ 'sb1', 'sb2', 'next-posts', 'single', 'blog', 'contact' ];
			update_option( 'sidebars_widgets', $sidebars_widgets );
			update_option( GULIR_TOS_ID, $theme_options );
			...
			register_sidebar( [
				'name'          => 'next-posts',
				'id'            => 'gulir_sidebar_multi_next-posts',
				'before_widget' => '<div id="%1$s" class="widget %2$s">',
				'after_widget'  => '</div>',
				'before_title'  => '<h3 class="widget-title">',
				'after_title'   => '</h3>',
			...
			register_sidebar( [
				'name'          => 'contact',
				'id'            => 'gulir_sidebar_multi_contact',
				'before_widget' => '<div id="%1$s" class="widget %2$s">',
				'after_widget'  => '</div>',
				'before_title'  => '<h3 class="widget-title">',
				'after_title'   => '</h3>',
		...
		function register_podcast() {
			...
			if ( class_exists( 'Gulir_Register_Podcast' ) ) {
				Gulir_Register_Podcast::get_instance()->register();
			}
		}
		...
		public function get_import_progress() {
			...
			wp_send_json_success(
				[
					...
					'label'   => get_option( self::$progress_label_key, esc_html__( 'Installing...', 'gulir-core' ) ),
				], 200
			);
