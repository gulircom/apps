...
if ( ! class_exists( 'rbSubPageAdobeFonts' ) ) {
	class rbSubPageAdobeFonts extends RB_ADMIN_SUB_PAGE {
		...
		public function set_sub_page() {
			...
			$this->page_title = esc_html__( 'Adobe Fonts', 'gulir-core' );
			$this->menu_title = esc_html__( 'Adobe Fonts', 'gulir-core' );
			...
			$data['button'] = esc_html__( 'Save Changes', 'gulir-core' );
			$data['delete'] = esc_html__( 'Delete Project', 'gulir-core' );
					...
					if ( isset( $font['variations'] ) && is_array( $font['variations'] ) ) {
						foreach ( $font['variations'] as $variation ) {
							if ( substr( $variation, - 1 ) === 'i' ) {
								$label = $font['family'] . esc_html__( ' - Italic', 'gulir-core' ) . ' ' . substr( $variation, 0, - 1 );
							} else {
								$label = $font['family'] . esc_html__( ' - Normal', 'gulir-core' ) . ' ' . $variation;
							}
