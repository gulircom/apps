	...
	class Ruby_Ajax_Adobe_Fonts {
		...
		public function font_api( $project_id ) {
			...
			if ( is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) !== 200 ) {
				return esc_html__( 'Project ID is invalid. Please check again!', 'gulir-core' );
			}
			...
			if ( ! count( $data ) ) {
				return esc_html__( 'Project is empty. Please add some fonts to your project.', 'gulir-core' );
			}
		...
		function remove_font() {
			...
			wp_send_json_success( esc_html__( 'Adobe Fonts have been successfully removed.', 'gulir-core' ) );
		...
		function update_font() {
			...
			if ( empty( $project_id ) ) {
				wp_send_json_error( esc_html__( 'Project ID not found.', 'gulir-core' ), 400 );
			...
			wp_send_json_success( esc_html__( 'Adobe Fonts have been successfully loaded.', 'gulir-core' ) );
