...
if ( ! class_exists( 'rbSubPageTranslation' ) ) {
	class rbSubPageTranslation extends RB_ADMIN_SUB_PAGE {
		...
		public function set_sub_page() {
			...
			$this->page_title = esc_html__( 'Quick Translation', 'gulir-core' );
			$this->menu_title = esc_html__( 'Quick Translation', 'gulir-core' );
			$this->position   = 20;
		...
		public function generate_data() {
			...
			$files  = [
				GULIR_CORE_PATH . 'languages/' . $prefix . 'gulir-core.pot',
			];
				...
				if ( file_exists( $file ) ) {
					...
					if ( ! empty( $translations ) ) {
						foreach ( $translations as $str ) {
							if ( ! empty( $str ) ) {
								$str_id = gulir_convert_to_id( $str );
								array_push( $data, [
									'id'  => $str_id,
									'str' => $str,
								] );
